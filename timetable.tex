...
{%
    \begin{#1}[#3]{#4}
    % \noalign{\createIsWorkdayCommand{#5}{#4}}
}
{\end{#1}}
...
%  {
%     #2
%     % Split the header specification string (#2) into a sequence using space as delimiter.
%     \seq_set_split:Nnn \l_tmpa_seq { ~ } { #2 }
%     % Compare the element at index #3 with I.
    
%    \cs_new:Npn \createIsWorkdayCommand:m{ \str_if_eq:nnTF { \seq_item:Nn \l_tmpa_seq {#2} } { I } {Ipa} {Ferie}}

%   % #1 is \noColls which originates from Timetable
%   % update this line so the current #2 (the header args) gets passed and then the function should take an argument and should return if the
%   % index of the string(header args) split by " " is A
