\ExplSyntaxOn

\cs_new:Npn \getLetter{H} % Example function returning 'A'

\cs_gset:Npn \runForDays #1#2#3
{
  \seq_set_split:Nnn \l_tmpa_seq { ~ } { #1 }
  \seq_map_inline:Nn \l_tmpa_seq
    {
      \str_case:nn {##1}
        {
          {W}{#2}
          {R}{#3}
        }
    }
}

\cs_gset:Npn \createDayTimeSlots #1
{
  \runForDays{#1}{\aDay{123}}{& 0 \insertRestDaySpace}
}

\cs_gset:Npn \createDayHeader #1
{
  \runForDays{#1}{\ipaDayHeader{1.1.190}}{\holidyDayHeader{}}
}

\cs_gset:Npn \calcDays #1
{
  \runForDays{#1}{\addIpaDay}{\addRestDay}
}

\cs_gset:Npn \getDayIndex #1
{
  \int_div_truncate:nn {#1}{\blockAmountPerIpaDay} + 1
}

\cs_gset:Npn \insertRestDaySpace{
  \Repeat{\hiddnRestColls}{&}
}

\cs_gset:Npn \timeslotItem #1#2#3{
  \bool_while_do:nn
    {
      \str_compare_p:eNe { \getDay{\arabic{#1}} } = { R }
    }
    {
      \Repeat{\blockAmountPerIpaDay}{\addtocounter{#1}{1}&}
    }

    % \Greencells{#2}{#2 (\arabic{#1}) ()} &
    \Greencells{#2}{(#3) (\blocksTilNextRest{#1})} &
}

\NewDocumentCommand{\createGetDay}{m}{
  \cs_gset:Npn \getDay ##1{
    \tl_item:nn {#1}{\getDayIndex{##1}}
  }
}

\NewDocumentCommand{\createTimeslot}{m}{
  \int_new:N \g_slots_to_draw
  \int_new:N \g_drawn_slots
  \cs_gset:Npn \timeslot ##1##2{
    \int_compare:nNnT {\arabic{##1}} > {0} {
      \int_gset:Nn\g_slots_to_draw{\number##2}%
      \int_gset:Nn\g_drawn_slots{0}%
      % \int_gset:Nn\g_drawn_slots{\number##2}%
      \emptycells{\arabic{##1}}
    }

    % \int_do_until:nNnn {\g_drawn_slots} = {\g_slots_to_draw} {
      % \int_compare:nTF {\g_slots_to_draw > \blocksTilNextRest{##1}}
      % {
      %   % \timeslotItem{##1}{\blocksTilNextRest{##1}}{##2}
      %   \int_gset:Nn\g_slots_to_draw{\int_eval:n{\g_slots_to_draw -\blocksTilNextRest{##1}}}
      % }
      % {
      %   % \timeslotItem{##1}{\int_use:N \g_slots_to_draw}{##2}
      %   \int_gset:Nn\g_slots_to_draw{0}
      % }
    % }
        
    \timeslotItem{##1}{##2}{##2}

  
    \emptycells{\numexpr\number\blockAmount-\arabic{##1}- (##2) -1\relax} 
    \addtocounter{##1}{\number##2}
  }
}

\NewDocumentCommand{\createBlocksTilNextRest}{m}{
  \newcouterinit{till_next_rest}
  \int_new:N \l_till_next_rest
  \cs_gset:Npn \blocksTilNextRest ##1{
    \int_gset:Nn \l_till_next_rest {\arabic{##1}}
  
    
    \bool_while_do:nn
    {
      \str_compare_p:eNe { \getDay{\number\numexpr\arabic{##1} + \l_till_next_rest\relax} } = { W }
    }
    {
      \int_gset:Nn \l_till_next_rest {\int_eval:n { \l_till_next_rest + 1 }}
    }
    \int_use:N \l_till_next_rest
  }
}

\ExplSyntaxOff{}